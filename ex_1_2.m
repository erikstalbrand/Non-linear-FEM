%% %% Exercise 1.2 a) Fully linearized method
clc;
clear
close all;
...
%% task 2. b) Fulfill constraint equation
clc;
clear
%close all;
...
for n = 1:path_steps
    ...
    while abs(res) > TOL || iter == 1      % Equilibrium iteration
        disp(['Residual: ', num2str(abs(res))]);
        ...
        if iter == 1            % The first iteration we need to calculate dlambda differently due to denominator in c will become zero
            if n > 1
                s = sign(du_tot*du_p+dP_tot);
                %s = sign(du*du_p + dlambda);       VARFÖR FUNKAR INTE DENNA???
                dlambda = s*(l/sqrt(du_p^2+fi*p^2));
            else
                dlambda = l/sqrt(du_p^2+fi*p^2); % s = 1 i första iterationen
            end
            ...
            % Load parameter
            a1 = du_p^2+fi*p^2;
            a2 = 2*du_p*(du_tot+du_r)+2*fi*dP_tot*p;
            a3 = (du_tot+du_r)^2+fi*dP_tot^2-l^2;
            a4 = du_tot*(du_tot+du_r)+dP_tot^2;
            a5 = du_tot*du_p+dP_tot*p;
            % Solve for dlambda
            eq = [a1 a2 a3];
            dlambdas = roots(eq);
            
            % Two solutions --> must choose the right one
            if a4 + a5*dlambdas(1) > 0
                dlambda = dlambdas(1);
            else
                dlambda = dlambdas(2);
            end
        ...
        %         out = imag(dlambda) ~= 0;
        %         if out == true % then dlambda is imaginary
        %             break %Decrease loadstep and restart iteration
        %         end
...
xlabel('Displacement')
ylabel('Force')
legend([p1 p2], 'Constraint enforce', 'True path')
xlim([0 2])