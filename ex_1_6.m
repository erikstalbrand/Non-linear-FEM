%% Exercise 1.6 Fully linearized method
clc;
clear
close all;
...
for n = 1:path_steps
    ...
    while abs(res) > TOL || abs(c) > cTOL || iter == 1      % Equilibrium iteration
        ...
        A = 1 - 2*y0*(u-P/X1)+(u-P/X1)^2;
        Kt = 2*((-(1/2)/sqrt(A)*(-2*y0+2*u-2*P/X1))*(y0-u+P/X1)-(1/lambda-1));
        ...
        lambda = sqrt(1 - 2*(u-P/X1)*y0 + (u-P/X1)^2);
        f_int = 2*(1/lambda - 1)*(y0-(u-P/X1)); % + kbar*u;
        f_ext = P;
